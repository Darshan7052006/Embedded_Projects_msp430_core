...
{
    return (adc_value * 3.3f) / 1023.0f;
}

float temperature_read(float voltage)
{
    return (voltage / 0.01f) - TEMP_OFFSET;
}
...
int main(void)
{
    ...
    while (1)
    {
        ...
        if (!(P2IN & START_BUTTON))
        {
            ...
            while (1)
            {
                current_button = (P2IN & START_BUTTON);
                if ((previous_button != 0) && (current_button == 0))
                {
                    delay(20);  
